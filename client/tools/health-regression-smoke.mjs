...
async function runSmoke() {
  ...
  try {
    ...
    if (!pos) {
      ...
      const dxz = Math.hypot(pos.x, pos.z);
      log(`POSITION: Tab A's view of remote = (${pos.x.toFixed(3)}, ${pos.y.toFixed(3)}, ${pos.z.toFixed(3)})`);
      ...
      if (dxz > RESPAWN_SLACK_M) {
        errors.push(`[position-not-reset-XZ] remote XZ distance from origin = ${dxz.toFixed(3)}, expected within ${RESPAWN_SLACK_M}m of (0,0)`);
      }
