...
function makePlayer(
  ...
  return {
    ...
    positionX: x,
    positionY: y,
    // PR #156 — vertical Y. Tests default to ground level (1.0 =
    // capsule half-height). Specifics at the test site override
    // this with `state.positionZ = ...` where needed.
    positionZ: 1.0,
    velocityX,
    velocityY,
    yaw: 0,
...
describe("remoteInterpolator PR 11.7.C — interpolation + extrapolation", () => {
  ...
  it("Test K: tick(now) returns a Babylon Vector3 per remote player (local excluded)", () => {
    ...
    const snap1: Snapshot = {
      ...
      nextServerFrame: 105,
      players: [
        { playerId: 1, positionX: 0, positionY: 0, positionZ: 1.0, velocityX: 0, velocityY: 0, yaw: 0, pitch: 0, hp: 100, ammo: 0, isFiring: 0, weaponId: 0, currentFireMode: 0 },
        { playerId: 2, positionX: 5, positionY: 0, positionZ: 1.0, velocityX: 0, velocityY: 0, yaw: 0, pitch: 0, hp: 100, ammo: 0, isFiring: 0, weaponId: 0, currentFireMode: 0 },
      ],
    };
    const snap2: Snapshot = {
      ...
      players: [
        { playerId: 1, positionX: 0, positionY: 0, positionZ: 1.0, velocityX: 0, velocityY: 0, yaw: 0, pitch: 0, hp: 100, ammo: 0, isFiring: 0, weaponId: 0, currentFireMode: 0 },
        { playerId: 2, positionX: 7, positionY: 0, positionZ: 1.0, velocityX: 0, velocityY: 0, yaw: 0, pitch: 0, hp: 100, ammo: 0, isFiring: 0, weaponId: 0, currentFireMode: 0 },
      ],
    };
