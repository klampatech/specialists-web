...
|**`You are here`**: post-PR-#71 (2026-08-28). **`main` @ `aef842f` (PR #71 squash — MERGED 2026-08-28).** PR #71 wires `client/tools/cross-machine-smoke.mjs` into `.github/workflows/ci.yml` as a **REQUIRED CI check**. **CI: 26/26 GREEN** (was 24/25 because the cross-machine smoke was new; second CI run after fixing the teardown hang + NAV_HOST for CI runners was 26/26 clean). The smoke uses its built-in **m5-headless Tab B fallback** in CI (no MacBook access; `NAV_HOST` = `127.0.0.1`; `KYLAMPA_SSH_PASSWORD=""` disables the SSH path). Real cross-machine validation still works locally via `KYLAMPA_SSH_PASSWORD=<pw> node client/tools/cross-machine-smoke.mjs` (verified on real m5+MacBook in PR #68). **Key fixes in PR #71** that landed: (1) `MACBOOK_SSH_PASSWORD` now `null` when env var unset (was hardcoded placeholder; would silently use wrong password in CI if `sshpass` was installed). (2) `isMacbookReachable()` short-circuits to `false` when password is null. (3) `NAV_HOST` and `SANS_EXTRA` are conditional — local dev uses Tailscale IP for cert SANs + navigation URL; CI uses 127.0.0.1. (4) Cross-machine smoke now calls `process.exit(0)` after writing `cross-machine-summary.json` (fire-and-forget teardown) — fixes the ~160s post-success hang that CI's shell `timeout 180` was masking. (5) CI step wraps the smoke in `timeout 180` as outer belt-and-suspenders + exit code propagation. **This is now the gate** for Kyle's "stable across all scenarios" criterion (cc: 1542549692896772196). **PRs #64-#71 all MERGED. None outstanding.** **Carry-forward rules**: (1) **smoke harness with console + server-log + DOM capture is the standard** — every new smoke uses `attachSmokeCapture` (or imports `log/fail/sleep` from `smoke-capture.mjs`). (2) **Pointer-lock gating kills headless yaw** — `__dragYawMode` flag bypasses (smoke-only). (3) **CI-FLAKE:CF-N1 (HP-convergence mpsc-saturation race)** — empty commit + push recipe confirmed working. (4) **Live pilot via SSH + CDP tunnel works** — pattern in `cross-machine-browser-validation` skill. (5) **CF-2026-08-27.A — RESOLVED in PR #70** — `two-tab-manual-flow.mjs:425` now uses magnitude (√(Δx² + Δz²)) instead of `Δx`. (6) **Cross-machine smoke teardown hang — RESOLVED in PR #71** — `process.exit(0)` after success, fire-and-forget teardown, shell `timeout 180` outer wrapper. **Servers all shut down clean.**

|**Recommended next PR** (Priority 1, ~30 min): **PR 72 — `fix(ci): pre-bake canary dev cert + ungate two-tab-manual-flow from continue-on-error`** — the manual-flow smoke's `continue-on-error: true` is paired with a cert-flakiness comment ("CI's slow dev-cert generation (~75s) races the smoke's canary-wait timeout, making this job flaky on a cold runner"). Now that PR #70 fixed the walk assertion AND PR #71 verified the wire path on the same cert infrastructure, the cert cache work is the remaining piece. Pre-bake the dev cert at job setup, ungate to required, becomes another regression gate. **Priority 2 (~30 min):** **PR 73 — `fix(health-regression): connectionStatus-drift vitest** — PeerOverlay/App.HUD state-machine lags when transport changes mid-frame. **Defer:** NB-3 from PR #56 (R-keypress needs a real-browser tier-3 test, same pattern); remote rig collision (blue-rig clips through boxes), anti-cheat on yaw/pitch (Phase 4 / PR 11.10), server-side hit detection refinement (hitbox lag-comp + multi-bullet), PR `0x0B MeleeEvent` future wire type (Phase 2).
