...
1. **`client/src/net/ggrsRuntime.ts`** — added `ROLLBACK_CAP_FRAMES` re-export, `paused` flag on `AdvancedFrame`, cap check in `advanceFrame()` using the `predictionDepth` formula (`max(0, localFrame - 1 - highestRemoteFrameSeen)`) — **GATED on `highestRemoteFrameSeen >= 0`** so the cap never fires on a solo browser (no peer to "fall behind" means no leading). `_pausedFrames` + `_totalPausedFrames` counters, `isPaused`/`pausedFrames`/`totalPausedFrameCount` public getters, counter-clear in `dispose()`. ~165 lines added. **Two bugs caught during Evo's re-verification**: (a) original codex draft used raw `localFrame - highestRemoteFrameSeen` (off by one at handshake + fires one tick too early); fixed to the predictionDepth formula. (b) Even with the predictionDepth formula, a solo browser with `highestRemoteFrameSeen=-1` self-pauses at advance #8 (because `localFrame - 1 - (-1) = localFrame` and `Math.max(0, localFrame)` is non-negative for any `localFrame > 0`). Without a peer there is NO notion of "falling behind", so the cap must be gated on `highestRemoteFrameSeen >= 0`. The first bug was caught at the smoke stage (off-by-one assertion); the second was caught by deterministic CI failure (5 of 16 smokes that drive the character via `tick()` regressed, none of the 7 smokes that don't drive `tick()` regressed — exact signature of the cap self-firing in solo mode). Fix is a 1-line addition to the cap check.
