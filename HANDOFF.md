...
**`You are here`**: post-PR-#129-followup (2026-09-05). **`main` @ `be57dd0` (PR #128 + #129-followup squash — MERGED 2026-09-05).** Multiplayer is end-to-end functional on Hetzner prod at `https://65.108.87.1:14432/?server=wss%3A%2F%2F65.108.87.1%3A14435%2Frooms%2F<id>&localId=1&peerId=2`. 2-tab damage convergence verified headless: 100 → 88 HP on hit, broadcastHandlerCount=1, both rigs visible in the peer viewport.

**The session shipped two PRs**:
- **PR #128 (`fix(matchmaker): --public-host flag`)** — matchmaker was returning `wss://<client-egress-IP>/...` from direct matchmaker consumers. Cloud deploys got URLs pointing back to themselves. Fix: `--public-host` flag threaded through `matchmaker.rs` / `transport.rs` / `canary-server.sh` with `peer.ip()` fallback for dev. Hetzner's `serve-static.mjs` URL-rewrite workaround is now a defensive no-op.
- **PR #129 (`fix: ungate scene.ts gameSession + two-tab prod-bundle damage smoke`)** — closed the dev-vs-prod gameSession tree-shaking gap (separate from PR #128's `await import()` variant). The first attempt (33ad83a, side-effect module) created a structural double-GameSession bug that **Claude Code's cross-vendor review caught** while my Stage 2 verification missed it (typecheck + vitest 118/118 + build + bundle-gate + prod-bundle-smoke 7/7 ALL green while multiplayer was broken). The actual fix (`be57dd0`): ungate `scene.ts`'s `__gameSession = gameSession` publication, delete `createGameSessionEntry.ts`, add **`client/tools/two-tab-prod-bundle-damage-smoke.mjs`** — the load-bearing gate for any future gameSession / wire-up integration change.

**Verification**: typecheck clean, vitest 118/118 PASS, cargo 119/119, `npm run build` clean, bundle gate: `__gameSession = x 1` (single instance), `ensureGameSession x 0`, `createGameSessionEntry x 0`. prod-bundle-smoke **7/7 PASS**. **NEW: two-tab-prod-bundle-damage-smoke 3/3 PASS** (`wire-up-connected-both-tabs`, `gameSession-single-instance-correct-ids`, `damage-converges-pre=100-post=88-drop=12-bcastCount=1`). Hetzner live: bundle `index-Bm8ArOAY.js` (md5 `3e6074df36f02ac81fba7dbcdc595080`) deployed via `scp + systemctl restart specialists-static`.

**Cross-vendor check-and-balance load-bearing (memory anchor §evo-verification-discipline-2026-09-05 + skill reference `pr-129-gameSession-tree-shake-and-cross-vendor-catch.md`)**: first-attempt verifier-clean ≠ correct. The smoke's `wire-up-connected === true` check passed 7/7 while multiplayer was non-functional. Counter-measure: re-run gates with "would the broken version pass this gate?" lens. For structural invariants (single-instance, shared state), structural verification (2-tab Playwright + assert HP convergence) beats signal-checking.
...
**The smoke** (`client/tools/cfn1-sustained-stress-smoke.mjs`, NEW, 570 LOC):
- Boots canary on 14449/14450/18087 + Vite on 5197 (next slot after crosshair per docs/PR-105-spec §2.4)
- 2-tab Playwright connects to a unique `CFN1_<ts>` room
- Smoke primer (no-op WeaponSwitch from each tab) + 1100ms rate-limit-window wait
- **12 assertions** (the strict CF-N1 regression signal is assertion 12 — Tab B HP dropped by ≥ 16 dmg from sustained spam)
- **Critical implementation detail**: strips ANSI escape codes (`\x1b\[[0-9;]*m` SGR sequences) from the canary log before regex matching. Tracing-subscriber emits color/bold formatting that breaks `drops_total=N` regex. Documented in the smoke header so future readers don't waste time on it.

**The CI gate** (`.github/workflows/ci.yml`, +74):
- New `client-cfn1-sustained-stress-smoke` job modeled on `client-stress-24p-smoke` (same canary+vite boot steps + same 3min timeout + same teardown pattern)
- **Opt-in** (`continue-on-error: true`) — full sustained-load smokes are expensive on CI runners. `damage-server-hp-convergence` stays as the required gate; the new smoke runs nightly to catch the CF-N1 class BEFORE the next CI run hits it.
- CANARY_STATS_INTERVAL_MS=2000 so we see ~5-7 [stress-stats] lines during the spam window
- cleanup-ports added 14449, 14450, 18087 to the post-job kill list

**PR #112's CI fix during flight** (commit `b2ffa1a`): the new CI job's `cargo build --manifest-path server/Cargo.toml` step failed with "manifest path does not exist" because the job's `defaults: working-directory: client` means cargo resolves the path relative to `client/`, not the checkout root. Fixed to `--manifest-path ../server/Cargo.toml`. The existing `client-two-tab-smoke` and `client-health-smoke` jobs use the same `--manifest-path server/Cargo.toml` pattern successfully — they don't hit this because their `cargo build` is wrapped in `(cd ../server && cargo build --release --bin specialists-server)` via `canary-server.sh`. The new job's direct cargo invocation didn't have that wrapper. Lesson encoded: for jobs with `working-directory: client`, the `--manifest-path` argument MUST be relative to that working directory (i.e. `../server/Cargo.toml`).

**Why PR #112 doesn't ship a structural fix to CF-N1**: PR #83 (per-room SnapshotGenerator) closed CF-N1 at the architectural root cause. PR #112 closes the **observation gap** — catches the flake via a dedicated sustained-load smoke. If the smoke fails on nightly, the next step is bumping the per-connection mpsc capacity (currently 1024) — but PR #112 doesn't pre-emptively bump because (a) it risks introducing different regressions (bigger memory footprint per room, more head-of-line blocking when fully saturated) and (b) the architectural fix from PR #83 should already prevent the flake from recurring under sustained load. PR #112 is **defensive observation**, not preemptive capacity bump.

**Verification**: cargo build clean (pre-existing async-fn lint warnings unrelated), cargo test --lib **119/119 PASS** (no server changes — this PR is observation-only, not a fix), vitest **104/104 PASS**, typecheck clean, `cfn1-sustained-stress-smoke.mjs` **12/12 PASS locally**, CI **35/35 GREEN** on the merge commit (the same recipe as PR #107+#108+#110's empty-commit re-trigger would have handled any CI flake if one had hit; the path-bug fix landed cleanly on the second run).

**No docs PR follows** — PR #112 is infra-only (smoke + CI gate, no behavioral change). Matches the precedent set by PR #99 (rig-visual-smoke-as-CI-gate) and PR #90 (orchestrator-smoke) which also didn't get docs PRs. Docs PRs follow behavioral PRs only (the post-#107/108/#109 and post-#110/#111 pairs were for behavioral changes).

**Carry-forwards beyond PR #112** (next play-testing blockers):
1. **`0x0B MeleeEvent` wire** (PR A in the play-testing arc) — melee is currently client-side only. Without server-authoritative melee, play testing will show weapon damage works but melee doesn't. Mirrors the PR #107+#108 wire-break pattern: server-side `MeleeEvent` (disc 0x0B + source u16 + yaw u16 + pitch u16 + eventId u32 = 11 bytes wire), `validate_and_relay_melee` with 5 gates (source-in-room, anti-spoof, rate-limit, melee-cooldown, distance/cone check). Client-side mirror: encodeMeleeEvent, F (or unused key) key handler, melee-damage applies via the existing snapshot HP path. New `client-melee-smoke` real-canary smoke + CI gate on unique ports. ~1 PR session.
2. **Pre-existing CF-N1 flake root-cause fix** (if the new opt-in smoke fails on nightly) — bump the per-connection mpsc capacity from 1024 to 2048 (2× bump, conservative) or add a snapshot coalesce (architectural hardening). Defer until the smoke actually fails on nightly — preemptive bump is not justified.
3. **Per-weapon icon sprites, Burst-active HUD polish, outbound mpsc review, new feature arc** — all unchanged from the post-#110 list, still deferred.

**Deploy coordination reminder**: PR #112 is server-only observation (no wire change). The existing wire-break deploy coordination for PR #107+#108+#110 is unchanged.

**Recommended next direction (your call)**:
- **(a) PR A: `0x0B MeleeEvent` wire** (~1 session, play-testing blocker). Closes the combat completeness gap (weapons + melee + reload). Single PR arc mirroring the PR #107+#108 pattern.
- **(b) Test the deploy end-to-end** (~30 min). Walk through the wire-break deploy message template (from docs/PR-105-spec §2.4), confirm the staging server has the new canary running, identify any kinks in the deployment pipeline before actual play testing. No code work.
- **(c) Outbound mpsc preemptive bump** (defensive, ~0.5 session). Bump 1024 → 2048, run the new opt-in smoke against the bump, observe behavior under sustained load. Pre-empts option (2C) before play testing starts.
- **(d) Maintenance sweep** — the few remaining deferred items from PR #94 + #92 reviews (focus-trap "soft" doc, popup-blocker flushSync parity, StrictMode rAF race — all cosmetic).

## 2026-09-02 — PR #114 MeleeEvent wire MERGED (`0x0B` server-authoritative melee — combat completeness gap closed)

**Scope**: Closes the `0x0B MeleeEvent` wire carry-forward from PR #110's recommended-next list (and the same item in the post-#112 TL;DR — option (a) "PR A: MeleeEvent wire"). The combat completeness gap is now closed: weapons (PR #107+#108) + melee (this PR) + reload (already server-authoritative from PR #83). **MERGED** at `6f723bd` (squash `6f723bd52d97d55660e6b1914bc3d44ed1a654ec`, merged 2026-09-02 20:52 UTC — squash of 12 in-flight commits). One PR, one squash, wire-break required (this is a new wire type, not an extension of an existing one).

**The 13-file diff** (+1640/-33):
- `protocol/damage.ts` (+122) — new `MeleeEvent` (0x0B) wire type: disc u8 + sourcePlayerId u16 + yawRadians u16 (fixed-point) + pitchRadians u16 (fixed-point) + eventId u32 = **11 bytes wire**. New `MeleeBroadcast` (0x0C): disc u8 + sourcePlayerId u16 + targetPlayerId u16 + damage u8 + headshotMultiplier u8 (HEADSHOT_MULTIPLIER constant when headshot, else 1) + eventId u32 = **8 bytes wire**. Reclaims DAMAGE_REJECT discriminator (was 0x0B in PR #59).
- `protocol/damage.test.ts` (+113) — 11 boundary tests for the new wire types.
- `server/src/protocol.rs` (+105) — wire decode for the new types.
- `server/src/damage_relay.rs` (+239/-X) — new `validate_and_relay_melee()` with **5 validation gates** mirroring the AimEvent gates: (1) source-in-room, (2) anti-spoof (source matches claim), (3) rate-limit (melee-cooldown), (4) cone-and-distance check (uses `hitscan::melee_cone_check`), (5) HP-decrement via existing snapshot HP path (PR #83's per-room SnapshotGenerator ensures the HP update reaches all subscribers).
- `server/src/hitscan.rs` (+60) — new `melee_cone_check()` for the cone-and-distance gate.
- `server/src/transport.rs` (+140) — dispatcher entries for 0x0B (MeleeEvent) and 0x0C (MeleeBroadcast). Reclaims 0x0B from DAMAGE_REJECT.
- `server/src/session.rs` (+10) — session-level melee state (last-melee timestamp for the per-player rate-limit gate).
- `client/src/net/damageBus.ts` (+84) — new `sendMeleeSwing({yawRadians, pitchRadians, frame})` API + `onMeleeBroadcast` listener. `nextMeleeEventId()` for monotonic eventIds (PR #114 smoke fix during flight).
- `client/src/net/serverTransport.ts` (+33) — wire encode/decode plumbing.
- `client/src/game/gameSession.ts` (+62) — wires F-key (or unused key) to melee swing; melee-damage applies via the existing snapshot HP path.
- `client/tools/melee-smoke.mjs` (NEW, 635 LOC) — real-canary smoke on unique ports. Boots canary+vite, two tabs connect to a unique `MELEE_<ts>` room. Multiple high-level assertions covering wire path + broadcast arrival + HP decrement + cone check.
- `.github/workflows/ci.yml` (+68) — new `client-melee-smoke` CI gate modeled on `client-weapon-switch-smoke` (same canary+vite boot, parallel with the rest of the smoke matrix).
- `.github/actions/cleanup-ports/action.yml` (+1/-1) — adds the 3 new canary ports to the post-job kill list.

**The in-flight fixes during PR #114** (commits `1185113`, `806e8ca`, `6fbd16d`, `601e951`, plus 6 others, all squashed):
- `b7ef9c8` fix(wire): melee HP decrement + smoke position race — the snapshot HP didn't decrement on melee hit because the smoke's `positionX/Y` was initialized to NaN, not a real position. Fixed in `hitscan::melee_cone_check` + smoke's seed.
- `6e5ef8a` fix(smoke): use `bus.nextMeleeEventId()` for monotonic eventIds — the smoke was generating its own random eventIds which could collide with server's running eventIds. Switched to the bus's monotonic counter.
- `4b94c1f` fix(smoke): drive melee via `__damageBus` (matches other smokes) — pre-fix the smoke was calling `sendMeleeSwing` directly via the transport, which bypassed the session state machine.
- `a69e65b` fix(smoke): use `canary-server.sh`'s actual flag names — flag mismatch was causing the canary to silently exit.
- `1185113` fix(smoke): 1.5s poll timeout + 6min CI cap — bounded the smoke's wait loops so CI doesn't hang on snapshot-population flakes.
- `806e8ca`, `6fbd16d`, `601e951` — Pitfall A1 (no explicit `process.exit(0)`) + A8 (double-teardown) prevention, mirroring the `weapon-switch-smoke.mjs` pattern from PR #110's lessons.

**Verification**: cargo build clean, cargo test --lib **119/119 PASS** (server-side tests for the new wire types + gate logic), vitest **110/110 PASS** (was 104 pre-#114; +6 from `damage.test.ts` 11 tests but pre-existing renames absorbed some; net +6), typecheck+build clean, `client/tools/melee-smoke.mjs` local pass, all **35/35 CI checks GREEN** on the PR #114 merge commit.

**Wire-break deploy coordination required**: PR #114 adds a new wire type (`MeleeEvent` 0x0B reclaims from DAMAGE_REJECT). All clients must refresh on deploy. The `next-deploy-is-all-clients-new` mitigation from docs/PR-105-spec §2.4 applies — see PR #115's deploy-prod.sh for the one-command deploy script.

**No docs PR follows PR #114 directly** — it landed during the overnight session which was already batching multiple changes; PR #115 (next entry) is the catch-all docs update that also records PR #114 retroactively.

**Carry-forwards beyond PR #114**: (a) **`tools/deploy-prod.sh` → actual wire-break deploy** of PR #107+#108+#110+#114 — the operational follow-through. (b) Pre-existing CF-N1 flake (carried from PR #112). (c) Maintenance sweep (carried from PR #112). (d) New feature arc (carried).

## 2026-09-03 — PR #115 overnight rollup MERGED (deploy-prod.sh + HUD icon-SVG swap + Burst-active chip + reconnect observability + A2 smoke fix)

**Scope**: Rollup of 5 commits that accumulated on local `main` ahead of origin during the overnight session + 1 smoke-fix commit (`6b871a8`) that surfaced on PR #115's first CI run. All additive/observational — no wire changes, no schema changes, no gameplay logic changes. **MERGED** at `43e4c04` (squash of 6 commits: `7c6a53a`, `8da9b12`, `ef265fe`, `3c85b68`, `d0b9271`, `6b871a8`, merged 2026-09-03 16:13 UTC).

**Why roll-up**: the 5 overnight commits were sitting on local `main` (5 ahead of `origin/main`) but had never been pushed or raised as PRs. When Kyle asked "where did we land after last night?", the honest answer was "5 commits on local main, no PRs awaiting review". Per Kyle's "do it right over do it fast" preference + the `cross-cutting-change-discipline` skill's guidance, we rolled them into one PR rather than raising them individually. Combined with the A2 smoke-fix commit (`6b871a8`) that surfaced on CI round 1, the final PR diff was **8 files, +518/-29**.

**The 5 overnight commits + 1 smoke fix**:

1. **`7c6a53a`** ops: `tools/deploy-prod.sh` (NEW, 126 LOC) — one-command wire-break deploy. SSHes to m5, pulls main, cargo builds, restarts the canary systemd unit. Flags: `--no-rebuild` (skip cargo), `--local` (run on this box, no SSH). Pre-flight checks (Tailscale up, Funnel on :14433, repo at `~/Development/specialists-web`). Companion to the `next-deploy-is-all-clients-new` mitigation in docs/PR-105-spec §2.4.

2. **`8da9b12`** fix(smoke): CF-N1 — poll for snapshot population, not one-shot read (A2 pattern). `client/tools/cfn1-sustained-stress-smoke.mjs` (lines 194-228 + assertion 1 at 325). Replaces a `sleep(1100) + single read` with `pollForBothPlayersInSnapshot(page, [1,2], 2000)` — polls every 50ms until both player IDs appear or 2s elapses. Closes the pre-#112 CF-N1 flake class.

3. **`ef265fe`** feat(ui): BulletHud Burst-active badge (`● FIRING`) when `isFiring=1 + fireModeIndex=Burst3`. Yellow chip rendered above the ammo count when the local player is actively firing in Burst mode. Replaces the silent cue from PR #108 (Burst consumed ammo without visible feedback).

4. **`3c85b68`** feat(ui): inline-SVG weapon icons (close PR #108 carry-forward). Replaces D/S/N ASCII letters in `BulletHud.tsx` with inline-SVG weapon silhouettes (~240 byte delta; same visual footprint as the ASCII letters). Per PR #108's recommendation: "real PNGs via `image_generate` (when fal.ai balance is back)"; this is the SVG-fallback path that doesn't require external image generation.

5. **`d0b9271`** feat(reconnect): reconnect observability in DebugHud (CF from PR #58). `client/src/net/serverTransport.ts` (+37) adds `reconnectAttempts` + `lastDisconnectAt` + `reconnectBackoffMs` to `ServerTransportStats`. `client/src/ui/DebugHud.tsx` (+21) renders "attempt #N · Xs ago · next in Ys" while reconnecting. Closes one of the 6 carry-forwards from PR #58's comment chain.

6. **`6b871a8`** fix(smoke): CF-N1 — same A2 poll pattern applied to `damage-server-smoke.mjs` (NEW smoke fix surfaced on PR #115's first CI run). Pre-fix the smoke did a one-shot read of `__latestSnap()` after ping/pong settled, racing the server's first 20Hz snapshot tick. CI round 1 failed with "pre-fire snapshot missing playerId=7 (single-tab ammo baseline)" — deterministic, not a flake. Same fix pattern as commit `8da9b12`, applied to a different smoke. Local verification couldn't run (port 8080 occupied in sandbox); CI was the primary verifier and it turned PASS.

**CI round history**:
- Round 1 (PR #115 trigger): 34/36 PASS, **2 FAIL** — `damage-server-smoke (5190)` "pre-fire snapshot missing playerId=7" (real regression, A2 pitfall) + `damage-server-hp-convergence (5191)` "fire-rate lower bound FAILED: 3 hits" (transient CF-N1 flake).
- Round 2 (rerun-failed-jobs): 35/36 PASS, **1 FAIL** — `damage-server-smoke (5190)` PERSISTENT after retry → real regression. Lobby smoke hung for 30+ minutes (likely unrelated CI runner flakiness on the heaviest smoke).
- Round 3 (after `6b871a8` + lobby retry): **36/36 SUCCESS**. ✓

**The A2-pitfall generalization**: any smoke that does a one-shot read of `__latestSnap()` after a transport-connect event is racing the server's first 20Hz snapshot tick. The fix is to poll until either the expected value appears or a 2-second budget elapses. **Two smokes now use this pattern** (cfn1-sustained-stress + damage-server). **Audit for other smokes with the same shape**: none found in the current smoke inventory, but any new smoke that reads `__latestSnap()` immediately after connect should use the poll pattern from the start.

**Verification**: cargo test --lib 119/119 PASS, vitest **118/118 PASS** (+8 from `serverTransport.test.ts` 9 tests + `BulletHud.test.ts` 8 tests, net +8 after pre-existing renames), typecheck+build clean, `tools/damage-server-smoke.mjs` A2-fixed, **36/36 CI checks GREEN** on the merge commit.

**Wire-break deploy coordination**: PR #115 doesn't change the wire format. The deploy message for PR #107+#108+#110+#114 is unchanged — `tools/deploy-prod.sh` is the new way to ship it.

**Carry-forwards beyond PR #115**: (a) **`tools/deploy-prod.sh` → actual wire-break deploy** — first coordinated ship of PR #107+#108+#110+#114. ~30 min operational. (b) Pre-existing CF-N1 flake (carried from PR #112). (c) Maintenance sweep (carried). (d) New feature arc (carried). (e) Outbound mpsc review (carried).

**Recommended next direction (your call)**:
- **(a) Wire-break deploy of PR #107+#108+#110+#114** (~30 min, operational). `bash tools/deploy-prod.sh` on a host with Tailscale + SSH access to m5. Then share the Funnel URL with 2-3 friends for tier-3 play-testing. **Highest priority** — unblocks the play-testing arc that PR #112 set up.
- **(b) Outbound mpsc review** (~0.5-1 session, defensive). Drop-oldest + per-room SnapshotGenerator (PR #83) + producer-side rate-limiter (PR #81) cover the worst case. Pre-cloud verification pass + small capacity bump if needed before sustained cloud load.
- **(c) New feature arc** — MMR / region select / Discord OAuth, spectator mode, replay, scoreboard, leaderboard, anti-cheat. Phase 2 candidates that were deferred pre-#98.
- **(d) Maintenance sweep** — focus-trap "soft" doc, popup-blocker flushSync parity, StrictMode rAF race (all cosmetic) + tier-3 Vivaldi keyboard test.

## 2026-09-02 — PR #110 Crosshair MERGED (weapon-aware color + spread + recoil cue + new smoke + new CI gate)

**Scope**: First sub-deliverable of Phase 2.2. PR #110 ships the visible surface of the weapons wire that PR #107 + PR #108 made possible: a center-screen Crosshair component that reacts to the per-weapon tunables in `WEAPONS_TABLE` and to the local fire state. **MERGED** at `35fe26f` (squash `35fe26fe473f133bac6c413c511f6987473e4ea6`, merged 2026-09-02 16:13 UTC). One PR, one squash, no wire-break, no follow-ups needed on this branch — `next-deploy-is-all-clients-new` is not required because PR #110 doesn't change the wire format.

**The 7-file diff**:
- `client/src/ui/Crosshair.tsx` (+234, NEW) — DOM overlay, 4 line segments + center dot. `weaponColor()` (DualPistol grey, Shotgun orange, Sniper red). `spreadRadiusPx(accuracyDegrees)` sqrt curve (1.5°→~18px, 8°→~31px, 0.5°→~14px). `applyRecoil()` 1.6× while LMB held. Burst-active center dot 4px yellow when fire-mode is Burst AND firing. `pointerEvents: "none"` (PR 7.1 lesson). Exports helpers for vitest.
- `client/src/ui/Crosshair.test.ts` (+194, NEW) — 18 pure-logic boundary tests covering all helpers + `data-*` attribute contract pin.
- `client/tools/crosshair-smoke.mjs` (+540, NEW) — real-canary smoke on ports 14447/14448/18086/5196. 7 high-level assertions / 20 individual checks. Uses `session.tryStartWeaponSwitch()` + synthetic canvas mousedown (bypasses Playwright `keyboard.press` flakiness).
- `client/src/game/gameSession.ts` (+37) — new `lastInput` closure variable + `getFireHeld(): boolean` getter (the HUD's read-only surface into the session).
- `client/src/ui/App.tsx` (+26) — imports `Crosshair`. `HudState.fireHeld` polled from `session.getFireHeld()` at 10Hz. Renders `<Crosshair>` next to `<BulletHud>` (coherent data source).
- `.github/workflows/ci.yml` (+65) — new `client-crosshair-smoke` CI gate on the unique ports (next slot after weapon-switch).
- `.github/actions/cleanup-ports/action.yml` (+1/-1) — adds the 3 new canary ports to the post-job kill list.

**Verification**: cargo build clean (pre-existing async-fn lint warnings unrelated), cargo test --lib 119/119 PASS (no server changes), vitest **104/104 PASS** (was 86; +18), `tsc --noEmit -p tsconfig.app.json` clean, `crosshair-smoke.mjs` 20/20 PASS locally, **34/34 CI checks GREEN** on the merge commit.

**The double-teardown bug fixed in flight**: smoke's orchestrator was running `teardown()` twice (once in `runSmoke()`'s own finally, once in the outer wrapper's finally). On CI the second teardown killed an already-dead vite process and produced exit code 1 despite 20/20 PASS. Fixed in commit `f10af26` — mirrored the `weapon-switch-smoke.mjs` pattern (runSmoke returns boolean, orchestrator owns the process lifecycle). Lesson: any future smoke should follow the same pattern. Documented in the smoke's own header comment.

**Pre-existing CF-N1 flake**: damage-server-hp-convergence smoke hit the same `CI-FLAKE:CF-N1` mpsc-saturation pattern on PR #110's first run. Empty-commit re-trigger (same recipe as PR #107+#108) cleared it on the second run. Not introduced by PR #110 — PR #110 doesn't touch the mpsc code path.

**Carry-forwards beyond PR #110**:
1. **Weapon icon sprites** — D/S/N ASCII letters in BulletHud remain placeholder per PR #108's carry-forward. Real sprites via `image_generate` (when fal.ai balance resets). Polish PR when balance is back.
2. **Burst-active HUD polish** — the center dot is the only "Burst is consuming" cue; an HUD strip animation (e.g. ammo-bar yellow flash for the 3 burst rounds) could be a follow-up.
3. **Pre-existing CF-N1 flake** — outbound-mpsc saturation under snapshot pressure. Same empty-commit recipe handles it; eventual fix is bumping mpsc capacity.
4. **Open items from post-#110 recommendations** — outbound mpsc review, new feature arc (MMR / region select / Discord OAuth, spectator, replay, scoreboard, leaderboard, anti-cheat, `0x0B MeleeEvent`).

**Recommended next direction (your call)**:
- **(a) Weapon icon sprites** (~0.5 session) — replace D/S/N ASCII letters with real PNGs via `image_generate` (when fal.ai balance is back).
- **(b) HUD polish pass** (~0.5 session) — Burst-active ammo-bar flash + per-weapon fire-mode cycling cue in BulletHud + crosshair spread smoothing between weapon switches.
- **(c) `server/src/main.rs` outbound mpsc + back-pressure review** (~1 session, defensive) — pre-cloud verification pass + finally close CF-N1 by bumping mpsc capacity.
- **(d) New feature arc** — MMR / region select / Discord OAuth, spectator mode, replay, scoreboard, leaderboard, anti-cheat, `0x0B MeleeEvent` wire type.

**Deploy coordination reminder**: PR #110 doesn't change the wire format. The deploy message should mention the Crosshair is a new visible HUD element (player expectation: "I have a crosshair now"), but no client-refresh-required coordination needed.
