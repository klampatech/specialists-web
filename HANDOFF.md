...
**`You are here`**: post-PR-#110 (2026-09-02). **`main` @ `35fe26f` (PR #110 squash — MERGED 2026-09-02 16:13 UTC).** Phase 2.2 ships its first sub-deliverable: the Crosshair component. **`You are here` summary**: PR #107 (server: WeaponSwitch wire + Burst state machine + `is_firing` byte + `current_fire_mode` byte) and PR #108 (client mirror + 1/2/3 + B key handlers + per-weapon HUD + weapon-switch smoke + server `snapshot.rs` gap-fix) and PR #110 (Crosshair DOM overlay + weapon-aware color/spread + 1.6× recoil cue + 4px Burst-active center dot + vitest boundary tests + new crosshair-smoke + new CI gate) are all **MERGED** to `main`. The PR #107+#108 wire-break pair (`next-deploy-is-all-clients-new`) ships the weapons wire; PR #110 ships the visible surface of that wire (a center-screen crosshair the player sees). **Verification across the 3-PR arc**: cargo test 119/119 PASS, vitest **104/104 PASS** (was 86, +18 from `Crosshair.test.ts`), typecheck+build clean, weapon-switch-smoke 6/6 PASS, crosshair-smoke **20/20 PASS** (7 high-level assertions / 20 individual checks), **all 34 CI checks GREEN** on the PR #110 merge commit. **PR #110's CI fix during flight**: smoke's orchestrator was running `teardown()` twice (once in `runSmoke()`'s own finally, once in the outer wrapper's finally). On CI the second teardown killed an already-dead vite process and produced exit code 1 despite 20/20 PASS. Fixed in commit `f10af26` — mirrored the `weapon-switch-smoke.mjs` pattern (runSmoke returns boolean, orchestrator owns the process lifecycle). **Pre-existing CF-N1 flake** (`client — damage server HP-convergence smoke`) hit on PR #110's first run (same `CI-FLAKE:CF-N1` mpsc-saturation pattern from prior runs on `main`); cleared via empty-commit re-trigger (same recipe as PR #107+#108). Not introduced by PR #110. **Carry-forwards beyond PR #110** (deferred to future PRs, not wire-follow-ups): (1) Per-weapon icon sprites — D/S/N ASCII letters in BulletHud remain placeholder per PR #108 carry-forward; real sprites need `image_generate` (was locked at PR #108 time — same fal.ai balance issue). Polish PR when balance resets. (2) Burst-active HUD polish — the center dot is the only "Burst is consuming" cue; an HUD strip animation (e.g. ammo-bar yellow flash for the 3 burst rounds) could be a follow-up. (3) Pre-existing CF-N1 flake — outbound-mpsc saturation under snapshot pressure (logged `CI-FLAKE:CF-N1` in the smoke's own output; persistent-after-retry tag in PR #42's mpsc-capacity investigation); PR #110's code doesn't touch the mpsc path, but the same empty-commit re-trigger recipe handles it. (4) The two remaining open items from the post-#110 recommendations: outbound mpsc review + new feature arc (MMR / region select / Discord OAuth, spectator, replay, scoreboard, leaderboard, anti-cheat, `0x0B MeleeEvent`). No code work currently queued.
