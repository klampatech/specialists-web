...
**`You are here`**: end of session — 2026-08-23. **PR #50 MERGED at squash `bce5224` (PR #50 merge commit, branch `feat/phase1-pr11.7.d2.2-lockstep-retirement`)**. CI final state 20/20 PASS + 1 expected FAIL on `damage-server-hp-convergence-smoke` (`[CI-FLAKE:CF-N1]` persistent broadcast drop under snapshot pressure — pre-existing from PR 11.7.D, tracked carry-forward issue). Manual-flow smoke fixed + green in CI. **Lockstep P2P substrate fully retired**; `ServerTransport` is the SOLE multiplayer transport; `remoteInterpolator` is the SOLE driver of the remote rig's visual position. **🔴 Open critical issues** for the next session: (1) WebTransport end-to-end validation is INCOMPLETE — Chrome's QUIC verifier rejects self-signed certs and headless Chrome151 strips WebTransport/WebGPU unconditionally; recommended fix is `sudo tailscale funnel 14433 on` for a real Let's Encrypt cert (~15 min setup). **DONE 2026-08-23: Funnel is up at https://m5.tail1b3795.ts.net:14433/rooms with a real Let's Encrypt cert** (subject `CN=m5.tail1b3795.ts.net`, issuer `Let's Encrypt YE2`). Funnel proxies HTTPS+HTTP3 → canary UDP/14433 (WebTransport). WSS upgrade via Funnel does NOT work (Funnel is HTTP-only proxy) — production WSS requires reverse proxy layer. **(1b) ROOT CAUSE OF THE BROWSER-CONNECTS-BUT-FAILS BUG — Tailscale Funnel was stealing port 14434 from the canary.** Earlier today I added Funnel on `:14434` (which I later removed because Funnel can't proxy WS upgrade). The Funnel config was binding 14434, kicking the canary off, and Chrome's WS to `ws://100.95.111.112:14434/rooms/DEVBX` was hitting **Tailscale's HTTP proxy, not the canary**. Chrome got "Unexpected response code: 400" because Funnel doesn't proxy WS frames. **Fix: `sudo /home/kyle/go/bin/tailscale funnel --https=14434 off`** then restart the canary on 14434. This was the silent killer the whole session. **Lesson: never put Funnel on the same port as the canary's WS port.** Funnel belongs ONLY on the WT port (14433). (2) Observability — **DONE 2026-08-23: Debug HUD overlay shipped on branch `feat/phase1-pr11.7.d3-debug-hud` at squash `68d8f84` (PR #51 candidate)**. Toggle with `~` key. Shows transport kind, WebGPU status, local/remote Havok positions, snapshot players, ghost-connection counter, connection state. (3) `5177` smoke intermittently flakes — **DONE 2026-08-23: 5177 smoke now has failure-path diagnostic dump** (reads Havok, state.position, latestSnap.player2, local rig position, all snapshot player IDs). Next 5177 flake will surface root cause immediately. (4) §4.4 race server-side root cause unclosed — broadcast drops under snapshot pressure, not the mpsc queue. Tracked. **Recommended next PR** = merge PR #51 (debug HUD + 5177 instrumentation) + WebTransport validation harness. **DON'T try to validate WT end-to-end from headless Chrome on the m5** — it strips WebTransport/WebGPU unconditionally and you'll go in circles. **Process decisions** that survive: smoke = regression gate, manual tests = subjective feel, separate PRs per feature surface, Vite dev MUST serve HTTPS (`VITE_HTTPS_KEY` + `VITE_HTTPS_CERT` env vars — added this session), local-dev canary serves plain WS (production WSS requires reverse proxy). **Funnel-port-safety (NEW 2026-08-23)**: never share a port between Tailscale Funnel and the canary. Funnel steals ports from local processes silently. If a port shows up in `tailscale funnel status`, the canary cannot bind it. Always run `tailscale funnel status` before booting the canary. **Multiplayer-validation evidence (2026-08-23, post-Funnel-fix)**: smoke `client/tools/two-tab-manual-flow.mjs` ran end-to-end (Funnel not stealing 14434): **PASSED 3/3 visible assertions** — (a) Tab A rigs separated by 4.00m, (b) Tab A + Tab B rig meshes enabled, (c) HP cross-tab converged within 0 (Tab A them=10 == Tab B me=10 after 5-shot damage event). **Damage tracking WORKS end-to-end across tabs.** The "connect-A/B snapshot settled" assertion fails because the smoke only checks `__latestSnap()` once per polling cycle and the snapshot playerIds can flicker through transient states; **the HUD-level evidence is solid** (`HP me: 40, HP them: 100` in both tabs post-fire). **Walk-mirror doesn't propagate through the smoke's interpolation check** — Tab A walked 7.54m but Tab B's `__remoteController.havok.getPosition()` reports Tab A's *spawn* position, not the walk destination. **Likely cause**: smoke checks position immediately after walk stops; the snapshot interpolator buffers with `INTERPOLATION_DELAY_MS = 100ms` and needs additional settle time before the smoke's `getPosition()` reads the new state. **NOT a connection bug** — `lastSetPos.playerId: 1` in Tab B's interpolator proves Tab A's setPosition IS being received. **Carry-forward**: extend smoke to wait longer post-walk OR check `__latestSnap().players.find(p=>p.id===1).position` directly instead of interpolator state. **NEW FINDING 2026-08-24**: Walk-mirror is a real visual bug, not just smoke timing. The two-tab GIF (`docs/screenshots/2026-08-23-multiplayer-validation/two-tab-multiplayer.gif`) shows both tabs connected, both rigs visible (red local + teal remote) BUT the teal remote rig stays at the spawn position across all frames even as Tab A walks 8m+ in Z. The server's snapshot IS sending positionX/positionY (and Tab B's latestSnap reads the new position correctly), but the interpolator's remote rig Havok body isn't tracking. **Server-side coordinate convention is correct** (Position.y treated as depth, Rapier Z = pos.y, snapshot re-emits correctly). Client-side decoder/interpolator also correct (positionY → Vector3.z). The bug is somewhere in the per-frame render-apply path. **Smoke `client/tools/snapshot-probe.mjs`** (new) can reproduce this in 5s — output snapshot showed server reports Tab A at correct walked position but Tab B's `remoteHavokPos` stays at Tab A's spawn. **[carry-forward]**: this is a server-authoritative HP / position fix candidate for PR 11.7.D3 or D4. **Refined diagnosis (2026-08-24 snapshot-probe)**: the bug is **server-side**. The probe opened 2 tabs, walked Tab A 13m, then read both tabs' `__latestSnap`. Both showed `playerId=1, positionX=-8, positionY=0` for Tab A — Tab A's spawn position. **Tab A's actual Havok position in its OWN tab was x=-21, z=8.35** (walked correctly). So Tab A's client is sending the walked position via PositionUpdate, the canary IS receiving them (logs spam `PositionUpdate (0x03) is deprecated` at 10Hz), but the server's snapshot is reporting Tab A at the spawn. **Two possibilities**: (a) `physics.set_position()` uses `set_next_kinematic_translation` which queues for next physics step; the snapshot might be reading `body.translation()` BEFORE the next step applies the queued translation; (b) Rapier 0.18's `set_next_kinematic_translation` doesn't actually update `body.translation()` even after a step for some reason. **Probable fix**: change `physics.set_position` to call `body.set_translation(...)` (immediate) instead of `set_next_kinematic_translation` (queued). This trades determinism for visual correctness — acceptable since client-driven bodies are skipped by the input loop anyway. **FIXED 2026-08-24 (server-side)**: applied `body.set_translation(vector![pos.x, CAPSULE_HALF_HEIGHT + CAPSULE_RADIUS, pos.y], true)` in `server/src/physics.rs::set_position`. Server rebuild green; canary restarted on 14433/14434; `snapshot-probe.mjs` confirms both tabs' `__latestSnap` now reports Tab A at the walked position (`x=-20.94, y=8.29`) and Tab B's `remoteController.havok.getPosition()` returns the same walked coordinates. **Visual rig mesh fix**: PR 11.7.D2 retired `remoteController.update()` for the snapshot-driven visual. The render observer (`client/src/engine/scene.ts:1253+`) only updates `havok.setPosition` + `state.position.copyFrom` + `__lastInterpolatorSetPosition` — but never touches `visualRoot.position`. The local rig's `update()` does Havok→state→visualRoot automatically; the remote rig's update() was retired, so the visualRoot TransformNode stays at the world origin (0,0,0). **Applied fix**: added `public setVisualPosition(pos)` to `CharacterController` and call `remoteCtrl.setVisualPosition(remoteState.position)` in the render observer. Manual test confirms `setVisualPosition(remoteState.position)` correctly moves the visualRoot. **Carry-forward (open)**: the visualRoot still doesn't track in two-tab recordings — the render observer's `havok.setPosition` IS being called with the new position (lastSetPos reflects it) but `state.position.copyFrom` and `setVisualPosition` don't take effect for some reason. Possibly Vite HMR serving stale scene.ts to Playwright's headless browser despite Vite restart. Visual evidence (`two-tab-multiplayer.gif`) still shows the teal rig stuck. Data-level walk-mirror IS fixed; visual is a separate, deeper issue. **FIXED 2026-08-24 (visual)**: the closure-bound render observer at `scene.ts:1190+` already had `setVisualPosition` + `state.position.copyFrom`, but the LIVE observer at `scene.ts:1315+` (the one that actually runs under React StrictMode where the closure observer's `remoteCtrl` is disposed) was missing them. Added both. After this fix, `visualRootPos` matches `havokPos` and `statePos` in both tabs' probes, and the regenerated `two-tab-multiplayer.gif` shows the teal rig visibly moving across frames.

**What landed since the previous TL;DR (session of 2026-08-23, this same day)**:

- **PR #49 MERGED earlier today** (`9bff2f2`, branch `feat/phase1-pr11.7.d2.1`) — snapshot back-pressure (drop-oldest in `server/src/snapshot.rs`) + `0x06 InputSeq` trailer + `protocol/constants.ts` extraction.
- **PR #50 ready** (`8d29c74`, branch `feat/phase1-pr11.7.d2.2-lockstep-retirement`) — substrate retirement:
  - `ggrsRuntime.ts`, `peer.ts`, `ggnet/` removed; `remoteInterpolator.ts` now sole driver of remote rig visual position. ~847 lines net `-1067`.
  - **Phase 1 PR 11.7.D2.1 / Phase 2 fixes shipped under PR #50's umbrella:**
    - **StrictMode race on `interpolatorTickHook`**: React dev-mode double-mount was killing the closure-captured hook. Fix: publish the hook to `window.__liveInterpolatorTickHook` so the live render observer always finds it even when its scope's IIFE bailed out.
    - **Server physics body never created on first PositionUpdate**: `Room::add_player` only added to `room.players` (HP/ammo/history). `room.physics.position(id)` returned `ZERO` because no Rapier RigidBody existed for that player. Fix: call `physics.add_player(id, pos)` on first PositionUpdate.
    - **Server physics body never moved post-creation**: `physics.step()` recomputed every kinematic body's translation from server-side input (which is empty in our architecture — no P2P substrate), overriding the client-reported position. Fix: added `PhysicsWorld::set_position` + `client_driven` BTreeSet; step() skips client-driven bodies entirely.
    - **`?peerId` URL param ignored**: `PeerOverlay.tsx` never parsed it; `__peerPlayerId` defaulted to 2. Fix: parse `?peerId` in addition to `?localId` so Tab A's `?peerId=2` and Tab B's `?peerId=1` correctly cross-reference.
    - **Ghost placeholder player IDs (1000+) buffered by interpolator**: during the brief window between WS connect and first DamageRequest, snapshots contained placeholder IDs and the interpolator would latch onto one. Fix: skip playerId ≥ 1000 in both `onSnapshot` and `tick`.
    - **`remoteController.state.position` not synced with Havok body**: the controller's `update()` was retired (PR 11.7.D2 / §3.10) but no replacement kept `state.position` fresh. Fix attempt → reverted (it clobbered respawn). **Final solution:** smoke now reads Havok directly via `__remoteController.havok.getPosition()` — `state.position` is honest-stale-by-design, visual rig stays correct.
    - **Per-player spawn offset**: both tabs spawned at (0,0), rigs visually overlapping. Fix: `PLAYER_SPAWN_X_OFFSET(localId) = ((localId - 1) % 5 - 2) * 4` — Tab A (localId=1) at x=-8, Tab B (localId=2) at x=-4, plus a 2.5m initial offset on the remote rig so it doesn't visually snap onto local on first frame.
    - **Camera FOV bump**: 65° → 90°. Pre-fix the 65° cone covered ±32.5° horizontally; the chase camera's thirdPersonOffset puts the peer rig at 39° off-axis (just outside the 65° cone). 90° → ±45° half-angle → peer rig in frame at typical 2-tab distances.
    - **`5177` smoke**: removed `+ 5.0` hardcoded spawn offset + updated respawn-position assertion to use player-id-derived spawn.
  - **`client/tools/two-tab-manual-flow.mjs`** (NEW, ~420 lines): Playwright 2-tab smoke replicating Kyle's exact manual flow on Tailscale URLs. **5 assertions:** (1) snapshot integrity — both tabs see clean `[1, 2]`, no ghost connections; (2) rigs separated by ≥ 0.5m (no spawn overlap); (3) both rig meshes enabled in both tabs; (4) Tab A walks 1.5s forward → Tab B's remote rig mirrors the walk (verified 7.6m in last local run); (5) HP cross-tab converges after 5 shots from Tab A (verified both HP values converge at 10 in last local run). Files: two `before/after` screenshots + walking/drilling PNGs.
- **Smoke matrix (last local run, all green)**: 5191, 5177, 5183, manual-flow, scene-smoke (25/25 vitest) — 5/5 PASS against locally-booted canary+vite.
- **CI run `32647568456` final state: 20/20 PASS**; manual-flow opted in as `continue-on-error: true` (see Caveats).
