...
**`You are here`**: end of session — 2026-08-23. **PR #50 MERGED at squash `bce5224` (PR #50 merge commit, branch `feat/phase1-pr11.7.d2.2-lockstep-retirement`)**. CI final state 20/20 PASS + 1 expected FAIL on `damage-server-hp-convergence-smoke` (`[CI-FLAKE:CF-N1]` persistent broadcast drop under snapshot pressure — pre-existing from PR 11.7.D, tracked carry-forward issue). Manual-flow smoke fixed + green in CI. **Lockstep P2P substrate fully retired**; `ServerTransport` is the SOLE multiplayer transport; `remoteInterpolator` is the SOLE driver of the remote rig's visual position. **🔴 Open critical issues** for the next session: (1) WebTransport end-to-end validation is INCOMPLETE — Chrome's QUIC verifier rejects self-signed certs and headless Chrome151 strips WebTransport/WebGPU unconditionally; recommended fix is `sudo tailscale funnel 14433 on` for a real Let's Encrypt cert (~15 min setup). (2) Observability is a thin veneer — every bug this session was solved by adding a debug global + reading via `page.evaluate`. Need debug HUD overlay (toggle `~`) + `#[instrument]` per room in server transport/snapshot.rs/damage_relay.rs. (3) `5177` smoke intermittently flakes (1 PASS + 1 FAIL on consecutive local runs). (4) §4.4 race server-side root cause unclosed — broadcast drops under snapshot pressure, not the mpsc queue. **Recommended next PR** = "WebTransport validation" (Tailscale Funnel + non-headless browser probe + CI gate that asserts `activeKind === "webtransport"` in HUD probe). **DON'T try to validate WT end-to-end from headless Chrome on the m5** — it strips WebTransport/WebGPU unconditionally and you'll go in circles. **Process decisions** that survive: smoke = regression gate, manual tests = subjective feel, separate PRs per feature surface, Vite dev MUST serve HTTPS (`VITE_HTTPS_KEY` + `VITE_HTTPS_CERT` env vars — added this session). Full PR #50 implementation history in `docs/SPEC.md` under "2026-08-23 — PR 11.7.D2.1 / PR #50 implementation decisions".
