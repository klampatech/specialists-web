...
**`You are here`**: post-PR-#112 (2026-09-02). **`main` @ `886856d` (PR #112 squash — MERGED 2026-09-02 17:50 UTC).** Cloud-infra + play-testing work begins: PR #112 is the first sub-deliverable, closing the CF-N1 observation gap. **`You are here` summary**: PR #107 (server wire) + PR #108 (client mirror) + PR #109 (docs) + PR #110 (Crosshair component) + PR #111 (docs) + PR #112 (CF-N1 sustained-stress smoke + new opt-in CI gate) are all **MERGED** to `main`. **PR #112 is observation-only** (new smoke + new CI gate, no server or client behavioral change) — closes the observation gap on the CF-N1 flake that intermittently hits `client — damage server HP-convergence smoke` under CI runner pressure. The smoke drives the **EXACT** load profile that triggers CF-N1 (2-tab sustained spam-fire at 120ms cooldown over ~12s, 100 shots) and asserts the server's drop-oldest counter stays at 0 AND the rate-limited counter stays bounded AND HP actually drops on the target. **The new CI gate** (`client-cfn1-sustained-stress-smoke`) is **opt-in** (`continue-on-error: true`) — full sustained-load smokes are expensive on CI runners, so `damage-server-hp-convergence` stays as the required gate. The new smoke runs nightly to catch the CF-N1 class BEFORE the next CI run hits it. **Verification**: cargo test 119/119 PASS (no server changes), vitest 104/104 PASS (no client changes), typecheck+build clean, **cfn1-sustained-stress-smoke 12/12 PASS locally**, all 35 CI checks GREEN on the PR #112 merge commit. **PR #112's CI fix during flight**: CI YAML had `--manifest-path server/Cargo.toml` but the job's `working-directory: client` means cargo resolves the path relative to `client/` — "manifest path does not exist" error. Fixed in commit `b2ffa1a` to `--manifest-path ../server/Cargo.toml`. **No docs PR follows** — PR #112 is infra-only (matches the precedent set by PR #99 rig-visual-smoke-as-CI-gate and PR #90 orchestrator-smoke, which also didn't get docs PRs; docs PRs follow behavioral PRs only). **Carry-forwards beyond PR #112** (next play-testing blockers): (a) `0x0B MeleeEvent` wire (PR A in the play-testing arc) — melee is currently client-side only; without server-authoritative melee, play testing will show weapon damage works but melee doesn't. ~1 PR using the PR #107+#108 wire-break pattern. (b) Pre-existing CF-N1 flake root-cause fix (if the new opt-in smoke fails on nightly, the next step is bumping the per-connection mpsc capacity from 1024 — but PR #112 doesn't pre-emptively bump because it risks introducing different regressions). (c) Per-weapon icon sprites, Burst-active HUD polish, outbound mpsc review, new feature arc — all unchanged from the post-#110 list. **Deploy coordination reminder** for PR #107+#108 (the existing wire-break pair): deploy message must call out that ALL clients must refresh — existing tabs (cached JS from pre-#107) will be broken until reload because PlayerState grew 30 → 31 bytes and AimEvent grew 19 → 20 bytes.
