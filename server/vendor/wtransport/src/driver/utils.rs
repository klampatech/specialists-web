...
pub fn streamid_q2w(stream_id: quinn::StreamId) -> StreamId {
    ...
    // instead of the private `.0` field. quinn 0.11.9+ made
    // `StreamId.0` crate-private (see
    // https://github.com/quinn-rs/quinn/pull/2294), so the upstream
    // `stream_id.0` access fails to compile against current quinn
    // releases. The `From` impl is public and equivalent.
