...
|> **Current status (2026-08-15):** Phase 0 / Milestone 2 fully CLOSED. PR 11.1 + PR 11.2 series + PR 11.3 series + PR 11.4 all MERGED, 60s M2 stress test passed (PR #26 docs closure). **PR 11.5 — gap-bridging rollback cap — in flight** (this branch, `feat/phase0-pr11.5-rollback-cap`): replaces the documented-but-unused `MAX_PREDICTION_FRAMES = 8` constant with a load-bearing pause-when-too-far-behind cap. When `localFrame - highestRemoteFrameSeen >= ROLLBACK_CAP_FRAMES`, `LockstepRuntime.advanceFrame()` returns a sentinel `{paused: true, ...}` frame instead of advancing — both controllers + combat + bullet-time skip on that tick, but the wire encoder keeps firing so the peer can catch up. Reuses the existing 8-frame budget (~133ms @ 60Hz). New `client/tools/lockstep-rollback-smoke.mjs` (port 5188, 7 assertions: probe sanity, cap constant, within-cap advances, over-cap pauses, wire packets during pause, caught-up resume, monotonic total counter) + new `client-lockstep-rollback-smoke` CI job + new DEV probe `__lockstepProbe` in `scene.ts`. ~270 lines in `ggrsRuntime.ts` (~160 added) + ~88 lines in `gameSession.ts` (~88 added: `makeEmptyInputState` + `makeEmptyFrame` helpers + the paused-frame early-return) + 24 lines in `scene.ts` (the DEV probe). Wire format **UNCHANGED** — input packet remains PR 11.3's 12 bytes; the cap is purely local (the peer doesn't know we're paused, doesn't need to). Bundle delta: +1.06 kB raw (smaller than PR 11.4's +2.71 kB because no new chunks, just additions to `ggrsRuntime.ts`). **Next**: PR 11.6 — server-authoritative damage (Phase 1 work). Five new DEV probes (`__spectatorToggle`, `__spectatorProbe`, `__spectatorMoveDelta`, `__spectatorYawDelta`, `__spectatorPitchDelta`) + new `client/tools/spectator-camera-smoke.mjs` (9 assertions: initial state, F2 enter preserves position, move-delta moves position, yaw-delta + mod-2π wrap, pitch CLAMP at both ±π/2, WASD absorbed while spectator active, toggle off reattaches, yaw preserved across off/on cycles) + new `client-spectator-camera-smoke` CI job on port 5187. Production bundle: zero spectator code (Vite strips the `import.meta.env.DEV`-gated branches; verified by grep against `dist/assets/index-*.js` — no `__spectator` or `onSpectatorToggle` references in production bundle). Bundle delta: +2.71 kB raw / +0.5 kB gzip. **Unblocks every subsequent two-tab dev session** by giving us a way to inspect the world from outside — the cyan-rig-disappears issue was a dev-mode viewing problem, not a production camera problem. PR 11.3 series shipped per-player mouse pitch (vertical mouse-look) on top of PR 11.1's yaw: bytes 4-5 of the wire packet carry pitch as a little-endian uint16 ([-π/2, +π/2] range, ~0.00275°/LSB); chase camera applies pitch as a vertical tilt in both 1st-person and over-shoulder locked views (with backward-compat shim for pre-PR-11.3 zero-byte packets). PR 11.3 series includes four PRs (#20 pitch implementation, #21 Y-axis sign fix, #22 cross-context wire-format smoke, #23 docs verification). Three new CI smokes (mouse-pitch port 5184, pitch wire-format port 5185, pitch 2-tab wire-format port 5186) + extended pointer-lock-camera smoke with pitch assertions. PR 11.2 series shipped pause / loadout menu UI (Resume + viewMode preservation + ESC handling + 12-assertion smoke); residual ESC-equals-resume flicker tabled as known issue — see Known Issues section. **M2 row 10 (per-player first-person mouse-look) is now ✅ fully verified** (yaw + pitch on the wire, cross-tab propagation confirmed by Kyle's 2026-08-15 two-tab playtest + PR #22 cross-context smoke). **M2 row 11 (60s two-tab stress test) is now ✅ closed** (Kyle's 2026-08-15 dev-box playtest on http://100.95.111.112:5173/ — 60s of WASD + mouse-look + LMB + stunts on both tabs drove cleanly, zero console errors, frame-count delta within the documented no-rollback threshold, HP delta symmetric). **🎉 Phase 0 / Milestone 2 fully closed.** Next: (1) Gap-bridging rollback (PR 11.5, Phase 0 cleanup) → (2) Server-authoritative damage (PR 11.6, Phase 1).
