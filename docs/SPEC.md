...
|>
|> **Next**: PR 11.1 — **per-player first-person mouse-look** (the standard production camera model for internet multiplayer; pointer-locked yaw, click-to-lock, ESC-to-release, mouse-delta → yaw). Replaces the chase camera as the in-game view; chase camera becomes the fallback when pointer-lock is not granted (e.g., user has ESC'd, or the browser refuses pointer-lock for non-secure-context reasons). Affects `chaseCamera.ts` + `inputListener.ts` + a small `setYaw` plumbing change in `characterController.ts`. Medium-sized PR (3 files, ~80-120 lines net). Ships a new CI smoke (`client-mouse-look-smoke`) that confirms the camera yaw updates on mouse-delta events. Then PR 11.2 (dev-box free-fly spectator camera, F2 to detach, orbit with mouse, click to return — debug-only), PR 11.3 (gap-bridging rollback, the "pause-when-too-far-behind" cap in `ggrsRuntime.ts` to absorb the frame-count desync + Chrome tab throttling), PR 11.4 (server-authoritative damage, the first internet-multiplayer architecture step). Original PR 11 polish (real wall-detection via `PhysicsRaycast`, Mixamo glTF, kill-marker, hit-marker, death animation) queued after. See HANDOFF.md for the full next-session handoff. **Dev-box two-tab play visual context** (Kyle, 2026-08-13 18:30): cyan rig is often off-screen or hidden behind crates because the chase camera follows the local rig — this is the **correct** behavior for a per-player camera, just visually disorienting during dev-box play. PR 11.1 replaces this with first-person mouse-look (the correct production camera); PR 11.2 adds a spectator mode for dev-box viewing.
