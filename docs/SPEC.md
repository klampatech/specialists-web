...
| **PR 7** (combat semantics: dual-pistol raycast firing + tracer render, melee cone hit detection, per-client bullet-time scaling at 0.25x with air control). Wires the byte-1 FIRE / MELEE / BULLET bits that PR 6 reserved; byte-1 encoding was previously a no-op (decode always read `false`). Damage application is render-side log only (no real health pool mutation in this PR — that's PR 9+).
| **PR 8** (jump regression fix: gravity accumulation in the Havok controller's `update()`, tightened jump condition to require `vy ≤ 0`). Resolves the "holding Space makes the character fly up forever" bug observed during the dev-box playtest 2026-08-12. Ships a failing-test-first regression smoke (`client/tools/jump-regression-smoke.mjs`) that asserts holding Space for 2s produces one jump and returns to ground — the bug would have shipped otherwise.
| **PR 8.1** (wallrun rising-edge + cooldown guard). Follow-up to PR 8 after Kyle dev-box playtest 2026-08-13 found that holding Q mid-air made the character fly up indefinitely. Root cause: if `wallrunPressed` is true at any frame AFTER the wallrun 1000ms timer expires, the controller re-enters wallrun, resetting the timer — creating an indefinite upward cycle when real-browser auto-repeat fires keydowns faster than the wallrun duration. Fix: rising-edge detection (`wasWallrunPressedLast` field) + post-wallrun cooldown (`lastWallrunEndedAtMs + duration + 200ms` grace). Ships `client/tools/wallrun-regression-smoke.mjs` and a new `client-wallrun-smoke` CI job on port 5176.
|
