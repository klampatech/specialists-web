...
|>
|> **Next**: PR 11 candidates — Phase 1 polish ordered for **internet-multiplayer** (the project's actual goal): **(1) per-player first-person mouse-look** (pointer-locked yaw — the standard production camera model, like every other multiplayer FPS; the current chase camera is fine for dev but is the dev-box-viewing model not the shipped-player model), **(2) gap-bridging rollback so the frame-count desync stops growing** (real rollback via ggrs/wasm is the long-term answer; a simpler "pause-when-too-far-behind" cap in `ggrsRuntime.ts` is the first cut), **(3) server-authoritative damage** (current setup derives damage locally from lockstep — fine for LAN / Tailscale but doesn't survive 100ms+ WAN latency). Then the original PR 11 polish (real wall-detection via `PhysicsRaycast`, Mixamo glTF, kill-marker, hit-marker, death animation) + the **dev-box free-fly spectator camera** (F2 to detach from player, orbit with mouse, click to return — for two-tab debugging). See HANDOFF.md for the full next-session handoff. **Dev-box two-tab play visual context** (Kyle, 2026-08-13 18:30): cyan rig is often off-screen or hidden behind crates because the chase camera follows the local rig — this is the **correct** behavior for a per-player camera, just visually disorienting during dev-box play. A spectator camera mode fixes the dev-box viewing; per-player mouse-look fixes the production camera.
