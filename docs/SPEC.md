...
|> |> **Next** (post-PR 11.3 series, ordered by Kyle's priority + dependencies):
|>
...
|>
|> **(1) Dev-box free-fly spectator camera (PR 11.4, debug-only)** — F2 toggle detaches the camera from the player (free-fly, orbit with mouse, return on click). ~50 lines in `chaseCamera.ts`. Unblocks the next two-tab dev session (cyan-rig-disappears issue + world-outside-inspection).
|>
...
|>
|> **(3) Server-authoritative damage (PR 11.6, Phase 1, deferred)** — current setup derives damage locally from lockstep — fine for LAN / Tailscale but doesn't survive 100ms+ WAN latency. **Phase 1 work**, not Phase 0. Move damage application from `gameSession.tick` (per-client local) to a server-broadcast packet handler. This is the first step toward a real dedicated server.
|>
|> **(4) Original PR 11 polish (deferred)** — real wall-detection via `PhysicsRaycast`, Mixamo glTF, kill-marker, hit-marker, death animation. Cosmetic.
|>
|> **Recommended next session order**: **(0)** 60s M2 stress test → **(1)** spectator camera → **(2)** rollback cap. M2 closure comes first, then the dev-box unblock, then the WAN testability improvement.
