...
|> **Current status (2026-08-15):** Phase 0 / Milestone 2 / PR 17 + PR 11.2 series + **PR 11.3 MERGED**. PR 11.3 ships per-player mouse pitch (vertical mouse-look) on top of PR 11.1's yaw: bytes 4-5 of the wire packet carry pitch as a little-endian uint16 ([-π/2, +π/2] range, ~0.00275°/LSB); chase camera applies pitch as a vertical tilt in both 1st-person and over-shoulder locked views (with backward-compat shim for pre-PR-11.3 zero-byte packets). Two new CI smokes (mouse-pitch + pitch wire-format) + extended pointer-lock-camera smoke with pitch assertions. PR 11.2 series shipped pause / loadout menu UI (Resume + viewMode preservation + ESC handling + 12-assertion smoke); residual ESC-equals-resume flicker tabled as known issue — see Known Issues section. Next: (3) Spectator camera (PR 11.4) → (4) Gap-bridging rollback (PR 11.5).
...
|> **Next** (post-PR 11.1, ordered by Kyle's priority + dependencies):
>
|> **(1) Pause / loadout menu UI (PR 11.2 series)** — MERGED at https://github.com/klampatech/specialists-web/pull/18 (squash `80b2de1`). Shipped across PR 11.2 + 11.2.1 + 11.2.2 + 11.2.3. React overlay on ESC, Resume (re-locks pointer via browser API), Loadout + Settings placeholders, Disconnect Peer, menu orbit camera. Resume button works correctly. Residual known issue: ESC-equals-resume flickers under specific timing conditions (see Known issues below); tabled for follow-up per Kyle's 2026-08-15 call.
|>
|> **(2) Mouse pitch (PR 11.3) — MERGED.** Same wire-byte pattern as PR 11.1 yaw (bytes 4-5 = little-endian uint16, ~0.00275°/LSB). `MOUSE_LOOK.sensitivityRadPerPixel` applies to both `movementX` (yaw) and `movementY` (pitch). PR 7's `forwardFromYaw` is now `forwardFromYawPitch` (3D vector with Y component for vertical aim). Chase camera's 1st-person + over-shoulder render paths set `camera.rotation.x = -pitchRadians` (Babylon Y-up convention: positive rotation.x looks DOWN, so we negate to match "look up = positive pitch"). Menu orbit camera is unaffected (no pitch tilt). No determinism regression because pitch, like yaw, goes on the wire.
|>
|> **(3) Dev-box free-fly spectator camera (PR 11.4 candidate, debug-mode only)** — the cyan-rig-disappears issue in two-tab dev-box play is now BOTH (a) the cyan rig being off-screen (PR 11.1 + PR 11.3 partially help because each tab's camera is independent) AND (b) needing to inspect the world from outside (for visual debugging). Add an F2 toggle that detaches the camera from the player (free-fly, orbit with mouse, return on click). ~50 lines in `chaseCamera.ts`. Not a Phase 1 production blocker, but ships before the next two-tab dev session.
|>
|> **(4) Gap-bridging rollback (PR 11.5 candidate)** — ~70s frame-count desync between two tabs (no-rollback lockstep, repeated inputs fill the gap). Real rollback via ggrs/wasm is the long-term answer; a simpler "pause-when-too-far-behind" cap in `ggrsRuntime.ts` (if Tab A's frame > Tab B's frame + N, Tab A pauses until Tab B catches up) is the first cut. ~50 lines + new regression smoke.
|>
|> **(5) Server-authoritative damage (Phase 1, deferred)** — current setup derives damage locally from lockstep — fine for LAN / Tailscale but doesn't survive 100ms+ WAN latency. **Phase 1 work**, not Phase 0. Move damage application from `gameSession.tick` (per-client local) to a server-broadcast packet handler. This is the first step toward a real dedicated server.
|>
|> **(6) Original PR 11 polish (deferred)** — real wall-detection via `PhysicsRaycast`, Mixamo glTF, kill-marker, hit-marker, death animation. Cosmetic.
|>
|> **Recommended next session order**: **(3)** spectator camera → **(4)** rollback cap. Pitch is done (PR 11.3); the spectator camera unblocks the next two-tab dev session, rollback improves WAN testability.
>
