...
|>
|> - **PR 11.2.2 (READY for re-test, 2026-08-14)** — single-handler ESC architecture. **Status: open on PR #18 as commit `070df25`, all 11 CI checks green. Playtest ⚠️ UNVERIFIED.** Iteration on PR 11.2.1's ESC-equals-resume UX: the dual-handler architecture (inputListener's `onEscapePressed` + PauseMenu's useEffect keydown listener) caused a flicker in Kyle's playtest — when ESC was pressed while the menu was visible, both handlers fired `canvas.requestPointerLock()` in the same event-loop turn, the browser granted-then-revoked pointer-lock within ~16ms, and the menu briefly hid (rendering the locked camera at first-person offset) then reappeared (chase camera back in orbit mode, `menuAngle = 0` reset visible as "camera resets to its starting position"). Three intermediate attempts in PR 11.2.1 failed to suppress the flicker: removing the dual listener (caused a "menu-stays-open" regression where `requestPointerLock()` was apparently failing silently), adding `e.preventDefault()` + `canvas.focus()` before `requestPointerLock()` (did not change the observed browser behavior), and the `chase.isPointerLocked()` guard on `onEscapePressed` (also did not change the race). **PR 11.2.2's hypothesis (per Kyle's prescription)**: the dual-handler race was load-bearing because both code paths called `canvas.requestPointerLock()` from a keydown handler — Chrome's user-activation policy for ESC keydown → pointer-lock grant is unreliable, but click handlers on real `<button>` elements are explicit user activations and reliably preserve user-activation through the call chain. **Fix:** remove the ESC handler from `inputListener.ts` entirely (let the browser handle ESC natively when pointer is locked — it fires `pointerlockchange(false)` cleanly through the existing `onPointerLockChange` listener). Move ESC handling to a single `useEffect` keydown listener inside `PauseMenu.tsx` that's only registered when `visible === true`; the handler calls `onResume()` which routes through the React `<button>` `onClick` chain (single handler, single `requestPointerLock()` call, from a path that preserves user-activation).
|>   **Architecture now**:
