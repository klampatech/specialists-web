...
|> - **PR 10 + PR 10.1 + PR 10.2 stacked** (PR 10: first real health pool HP=100 + damage application + 1s respawn + HUD `HP me:` / `HP them:` lines; PR 10.1: WebRTC ICE candidate bundling fix; PR 10.2: separate `respawnPosition` from `startPosition` on `CharacterController` so the cyan rig respawns to (0, 0.9, 0) instead of (2.5, 0.9, 0)) — **code-complete in branch `feat/phase0-ice-candidate-bundling-rebased`** (off `feat/phase0-health-damage-respawn` @ `a853c8b`, which is off `main` @ `dab3c3e`). All 5 local gates green (typecheck + build + 5 smokes). **Covers Milestone 2 acceptance row 9** + fixes the PR 6 ICE-candidate regression + fixes the cyan rig respawn-position desync. No new wire byte — lockstep determinism guarantees identical damage application on both clients. Two-tab dev-box playtest 2026-08-13 confirmed cross-client HP drain + respawn sync working. Push + open a single combined PR + close #11 + #12 are the next steps. Ships new `client/tools/health-regression-smoke.mjs` + new `client-health-smoke` CI job on port 5177 + new `client/tools/pr10.1-connection-test.mjs` diagnostic tool. **Honest limitations carried into Phase 1 follow-ups:** (a) ~70s frame-count desync between two tabs (no-rollback lockstep, repeated inputs fill the gap); (b) cyan rig often hidden behind crates / off-screen because the chase camera follows the local rig (split-screen or shared chase camera is the fix); (c) Chrome tab throttling pauses backgrounded tabs' RAF loops (exacerbates the desync).
|>
|> **Next**: PR 11 candidates — Phase 1 polish (real wall-detection, Mixamo glTF, mouse-look, **split-screen / shared chase camera so the cyan rig is always visible during two-tab play**), PR 7.4 debug-instrumentation cleanup, or follow-up health items (server-authoritative damage, kill-marker, hit-marker, death animation, **gap-bridging rollback so the frame-count desync stops growing**). See HANDOFF.md for the full next-session handoff.
...
- **PR 10 (code-complete, all 6 local gates green; push + PR-open are the next steps):** first real health pool, damage application, respawn. **Covers Milestone 2 acceptance row 9** ("Health → 0 → 1s respawn timer → back at spawn"). Health lives on `CharacterController.state` (`hp`, `respawningUntilMs`); damage flows through the new `client/src/game/health.ts` (`applyDamage` + `tickRespawn`); respawn is `controller.respawn(nowMs)` which teleports the capsule to `SPAWN_POSITION` and clears the wallrun cooldown. Per-client symmetric: both controllers tick health + respawn every frame, damage from the rising edge of either input goes to the opponent. No new wire byte — the existing 8-byte `INPUT_SIZE` is unchanged. New `client/tools/health-regression-smoke.mjs` (single-tab Playwright, teleports the remote rig onto the local rig via `__teleportRemote` so every shot is a hit, drives 9 LMB hits to push remote HP from 100 → 0, asserts HP drains, respawn timer fires, HP restores to 100, position resets). New `client-health-smoke` CI job on port 5177. **Caveats caught by Evo's verification-gate re-verify after the codex lazy-stop:** (a) `getHealthSnapshot()` returns the *remaining* respawn countdown (`respawningUntilMs - lastNowMs`), not the absolute timestamp — the HUD label is honest now; (b) the smoke asserts `remote HP = 0` (the target), not `local HP = 0` (the firer).
...
### 2026-08-13 — Phase 1 follow-ups carried from PR 10 + 10.1 + 10.2
- **Cyan rig visibility during two-tab play (PR 11 candidate)** — the chase camera follows the local rig, so when the local rig walks away from spawn the cyan rig is often off-screen or hidden behind crates. Options: (a) split-screen (two cameras, two viewports, each half-width); (b) shared chase camera that frames both rigs in a single viewport (move camera to the midpoint, zoom out as rigs separate); (c) click-to-cycle which rig the camera follows. (a) is the cleanest first cut, gives both players equal screen real estate. **Not a bug per Kyle's 2026-08-13 call — it works, but the visual experience is sub-optimal. Filing as Phase 1 follow-up.**
- **Frame-count desync (PR 11 candidate)** — Tab A has run ~28,000 frames while Tab B has run ~26,000 frames. Both tabs agree on world state (HP, position) because the lockstep is deterministic and both compute the same result from the same input history, but their `frame` HUD counters differ by ~70s of game-time drift. This is the documented no-rollback lockstep limitation in `ggrsRuntime.ts` — repeated inputs fill the gap. **Phase 1 fix: real rollback (ggrs/wasm) or a simpler "pause-when-too-far-behind" cap.**
- **Chrome tab throttling** — when one tab is backgrounded, Chrome throttles RAF to ~1Hz, so that tab's simulation effectively pauses. The lockstep doesn't crash (it just runs slower on one side), but it exacerbates the desync. **Same Phase 1 fix as the frame-count desync.**
