...
|> - **PR 11.4 (MERGED — dev-box free-fly spectator camera, F2 detach, debug-only)** — second `UniversalCamera` instantiated lazily on first F2 press, reused across subsequent F2 toggles. F2 enters with the camera's *current world position* (no teleport); F2 again reattaches to the chase camera (lerp resumes normally). WASD flies at 8 m/s flat (faster than character walkSpeed=5); held-right-click-drag rotates (yaw + pitch, pitch clamps ±π/2). **Character controller's `controller.update(input)` is gated off while spectator active** — WASD absorbed so the character doesn't move. Smoke-driven regression: new `client/tools/spectator-camera-smoke.mjs` (9 assertions on port 5187) + new `client-spectator-camera-smoke` CI job. Bundle: +2.71 kB raw / +0.5 kB gzip. **Production bundle contains zero spectator code** — every spectator path is gated by `import.meta.env.DEV` (Vite statically replaces with `false` in production, Rollup eliminates the dead branch). Verified by `grep '__spectator\|onSpectatorToggle' dist/assets/index-*.js` → ZERO matches. The 36 "spectator" matches in the production bundle are Babylon.js internals (`_spectatorCamera`, `_spectatorAfterRenderObservable` — part of Babylon 9.20's XR feature), NOT our code. The 7 "F2" matches are React scheduler lane flags + Babylon's keyboard-mapping table; not our F2 spectator toggle. **Unblocks every subsequent two-tab dev session** by giving us a way to inspect the world from outside — the cyan-rig-disappears issue was a dev-mode viewing problem, not a production camera problem.

|> **Next** (post-PR 11.4, ordered by Kyle's priority + dependencies):
|>
|> **(0) 60s M2 stress test (no code, ~5 min playtest)** — formally closes Milestone 2 (last PENDING acceptance row in row 11). Both tabs driving simultaneously for 60s with periodic mouse-look + movement + shooting — watch console for errors, frame-count delta between tabs, HP delta for symmetric damage. **The 60s stress test is the highest-priority next item** because it's the only thing standing between M2 and "ready for Phase 1 work."
|>
|> **(1) Gap-bridging rollback cap (PR 11.5)** — ~70s frame-count desync between two tabs (no-rollback lockstep, repeated inputs fill the gap). Real rollback via ggrs/wasm is the long-term answer; a simpler "pause-when-too-far-behind" cap in `ggrsRuntime.ts` (if Tab A's frame > Tab B's frame + N, Tab A pauses until Tab B catches up) is the first cut. ~50 lines + new regression smoke.
|>
...
|>
|> **Recommended next session order**: **(0)** 60s M2 stress test → **(1)** rollback cap → **(2)** Phase 1 server work. M2 closure comes first, then the WAN testability improvement.
...
| One full minute of two-tab play = no console errors, no desync, no rubberbanding | Both tabs stay in sync for 60s | **PENDING** — last M2 acceptance row. Brief playtests through PR 11.1 (≤5 min each) confirm no errors + cross-client yaw propagation; the formal 60s stress test was deferred to dev-box session time. Run as the final M2 closure: both tabs driving simultaneously for 60s with periodic mouse-look + movement + shooting, watch console + frame-count delta + HP delta. **Closes M2.** |
