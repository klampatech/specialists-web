...
> **Current status (2026-08-14):** Phase 0 / Milestone 2 / PR 17 + PR 11.2 MERGED.
...
|>
|> - **PR 11.2** (pause / loadout menu UI — React overlay shown when pointer unlocks + everLocked, ESC-equals-resume, viewMode preservation across pause cycles) — **MERGED** at https://github.com/klampatech/specialists-web/pull/18 (squash commit `<filled at merge>`). New `PauseMenu` React component renders on `(!isPointerLocked) && everLocked`; cursor returns to default, semi-transparent dark overlay, 4 buttons (Resume / Loadout / Settings / Disconnect Peer). Resume calls `chase.setPointerLock(true)` — re-locks the cursor + restores the last viewMode. ESC-equals-resume (Kyle's spec): a `keydown` handler in `inputListener.ts` fires `onEscapePressed` to the host, which calls `chase.setPointerLock(true)`; UX matches every FPS (CS, Half-Life, Minecraft). ViewMode preservation: PR 11.1's `setPointerLock(true)` always landed at mode 0; PR 11.2 stores `lastLockedViewMode` and restores it on re-lock (default 0 for the very first lock). Loadout + Settings are `<button disabled>` placeholders with "coming soon" subtitles. Disconnect Peer calls `peer.close()` — re-host / re-join is PeerOverlay's job, kept separate. **Covers the second axis of the mouse-look UX loop** (the menu orbit camera + cursor unlock were placed by PR 11.1; PR 11.2 ships the menu the cursor was unlocking into). One new CI smoke (`pause-menu-smoke.mjs`, 12 assertions including explicit `pointer-events: auto` checks on backdrop + buttons — the HUD-overlay-eats-clicks trap from PR 7 burn) + one new CI job (`client-pause-menu-smoke` on port 5183). Also drops the stray `[input] window mousedown (capture path)` console.log from PR 7 that PR 7.4 cleanup missed. Bundle 7,048.62 kB → 7,052.19 kB (+3.57 kB raw / ~+0.5 kB gzip). All 9 smokes green on CI. Dev-box two-tab playtest required post-merge (smoke proves the React + chase-camera code paths; real-browser ESC → menu → Resume flow + the visual feel need a human).
|>
