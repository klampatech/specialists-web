...
|> **Current status (2026-08-15):** Phase 0 / Milestone 2 fully CLOSED. PR 11.1 + PR 11.2 series + PR 11.3 series + PR 11.4 + PR 11.5 all MERGED. PR 11.5 ships the **gap-bridging rollback cap** in `LockstepRuntime`: when `localFrame - highestRemoteFrameSeen >= ROLLBACK_CAP_FRAMES (8)` AND `highestRemoteFrameSeen >= 0` (the runtime has actually heard from a peer), `advanceFrame()` returns a sentinel `{paused: true, ...}` frame instead of advancing — both controllers + combat + bullet-time skip on that tick. Wire encode + submit still fires every tick so the peer can catch up. ~165 lines added to `ggrsRuntime.ts` (the cap check + `paused: boolean` flag on `AdvancedFrame` + `_pausedFrames`/`_totalPausedFrames` counters + `isPaused`/`pausedFrames`/`totalPausedFrameCount` public getters + counter-clear in `dispose()`) + ~88 lines in `gameSession.ts` (the caller-side early-return before controller update + combat, plus `makeEmptyInputState` / `makeEmptyFrame` helpers, plus `pausedFrames`/`totalPausedFrameCount` getters on the `GameSession` interface) + 24-line DEV-only `__lockstepProbe` accessor in `scene.ts` (tree-shaken in production; `grep '__lockstep\|ROLLBACK_CAP_FRAMES' dist/assets/index-*.js` → ZERO matches). New `client/tools/lockstep-rollback-smoke.mjs` on port 5188 (7 assertions: probe sanity, cap constant value, 9 within-cap advances, 5 over-cap pauses, 15 wire packets sent across all calls, caught-up resume, monotonic `totalPausedFrameCount`) + new `client-lockstep-rollback-smoke` CI job. Wire format **UNCHANGED** — input packet remains PR 11.3's 12 bytes; the cap is purely local (the peer doesn't know we're paused, doesn't need to). Bundle delta: +1.06 kB raw (smaller than PR 11.4's +2.71 kB because no new chunks, just additions to `ggrsRuntime.ts`). All 16 CI smokes green. Squash: `6e064e84`. PR #27. **Dev-box playtest DEFERRED to PR 11.6** — meaningful two-tab WAN-throttle test requires a real peer conversation + visible character animation state, neither available today (Kyle's call 2026-08-15: "tough to test this considering the models just look frozen in place anyway"). The cap is mathematically verified by the smoke (which seeds a synthetic peer packet and drives the scenario), the gate is verified by full CI (the original push regressed 5 of 16 solo-mode smokes before the `highestRemoteFrameSeen >= 0` gate was added — see decisions log for the bug). Real visual verification rolls into PR 11.6's checklist. **Next**: PR 11.6 — server-authoritative damage (Phase 1 work, ~2-4 sessions).
