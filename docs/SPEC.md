...
|> **Current status (2026-08-15 afternoon):** Phase 0 / Milestone 2 / PR 17 + PR 11.2 series MERGED. PR 11.2 series shipped pause / loadout menu UI (Resume + viewMode preservation + ESC handling + 12-assertion smoke); residual ESC-equals-resume flicker tabled as known issue — see Known Issues section. Next: (2) Mouse pitch (PR 11.3) → (3) Spectator camera (PR 11.4) → (4) Gap-bridging rollback (PR 11.5).
...
>
|> **(1) Pause / loadout menu UI (PR 11.2 series — OPEN on PR #18, residual known issue)** — Shipped across PR 11.2 + 11.2.1 + 11.2.2 + 11.2.3. React overlay on ESC, Resume (re-locks pointer via browser API), Loadout + Settings placeholders, Disconnect Peer, menu orbit camera. Resume button works correctly. Residual known issue: ESC-equals-resume flickers under specific timing conditions (see Known issues below); tabled for follow-up per Kyle's 2026-08-15 call.
|>
|> **(2) Mouse pitch (PR 11.3 candidate)** — PR 11.1 is yaw-only. Kyle hasn't asked for pitch yet, but it's the natural follow-up: same wire-byte pattern as yaw (bytes 4-5 = little-endian uint16 pitch, ~0.0055°/LSB), `MOUSE_LOOK.sensitivityRadPerPixel` applies to both `movementX` and `movementY`. PR 7's `forwardFromYaw` becomes `forwardFromYawPitch` and the camera's rotation needs both Euler-Y and Euler-X. No determinism regression because pitch, like yaw, goes on the wire and is decoded before WASD projection.
|>
|> **(3) Dev-box free-fly spectator camera (PR 11.4 candidate, debug-mode only)** — the cyan-rig-disappears issue in two-tab dev-box play is now BOTH (a) the cyan rig being off-screen (PR 11.1 partially helps because each tab's camera is independent) AND (b) needing to inspect the world from outside (for visual debugging). Add an F2 toggle that detaches the camera from the player (free-fly, orbit with mouse, return on click). ~50 lines in `chaseCamera.ts`. Not a Phase 1 production blocker, but ships before the next two-tab dev session.
|>
...
|>
|> **Recommended next session order**: **(2)** mouse pitch → **(3)** spectator camera → **(4)** rollback cap. Pitch rounds out the feel, the spectator camera unblocks the next two-tab dev session, rollback improves WAN testability.
>
...
| Per-player first-person mouse-look (per-player yaw, no shared chase camera) | Click canvas to lock pointer; mouse-delta rotates yaw; ESC releases and falls back to chase | **LANDED PR 11.1** ✅ — pointer-locked camera renders 1:1 with the character in 1st-person mode (no lerp); over-shoulder mode keeps the camera behind + slightly above the character with rotation tracking the character chest (model rotates IN VIEW). ESC releases into the **menu orbit camera** (slow auto-rotation around the character). V cycles between 1st-person-locked and over-shoulder-locked while locked; V is a no-op while unlocked. Yaw lives on bytes 2-3 of the input packet (`INPUT_SIZE` bumped from 8 to 10), little-endian uint16 (~0.0055°/LSB). Both clients compute identical WASD world directions from the same decoded yaw on the same frame — lockstep determinism preserved (the controller's `update()` calls `setYaw(input.yawRadians)` BEFORE projecting the character-relative WASD input into world space, so the authoritative yaw is the decoded-wire value, not a client-local accumulator). Sensitivity = 0.0025 rad/px (configurable via `MOUSE_LOOK.sensitivityRadPerPixel` in `characterConfig.ts`). Three CI smokes (mouse-look, pointer-lock-camera, yaw-wire-format) + dev-box two-tab playtest (Kyle, 2026-08-14) confirms click-to-lock, cross-client yaw propagation, over-shoulder model rotation, ESC menu orbit visible, V cycling correct. **Not in this PR**: pitch (deferred to PR 11.3 candidate). |
