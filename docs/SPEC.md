...
>
> **Next**: PR 10 — first real health pool, damage application, respawn (Milestone 2 row 9). Or Phase 1 polish items: real wall-detection via `PhysicsRaycast` for the Q-stunt, real Mixamo glTF character model, first-person mouse-look. Or PR 7.4 cleanup (remove PR 7.3's debug instrumentation — Kyle has confirmed combat is solid in real play post-merge). See HANDOFF.md for the full next-session handoff.
