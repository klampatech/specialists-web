...
|> - **PR 10** (first real health pool: HP=100 on `CharacterController.state`; damage application per rising-edge combat event to the opponent controller; 1s respawn timer driving a `controller.respawn()` teleport back to `SPAWN_POSITION`; per-client symmetric damage flow across both controllers; HUD `HP me:` / `HP them:` lines) — **code-complete in this branch** (off `main` @ `dab3c3e`, branch `feat/phase0-health-damage-respawn`); all 6 local gates green; push + PR open are the next steps. **Covers Milestone 2 acceptance row 9.** Ships new `client/tools/health-regression-smoke.mjs` + new `client-health-smoke` CI job on port 5177. No new wire byte — lockstep determinism guarantees identical damage application on both clients.
>
> **Next**: PR 11 candidates — Phase 1 polish (real wall-detection, Mixamo glTF, mouse-look), PR 7.4 debug-instrumentation cleanup, or follow-up health items (server-authoritative damage, kill-marker, hit-marker, death animation). See HANDOFF.md for the full next-session handoff.
