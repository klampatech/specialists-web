...
- **Smoke regression discipline caught a 1-tick-off cap math bug.** The initial smoke expected 7 within-cap advances + 8 catch-up packets, but Evo's Stage 2 re-verification (cap math trace) caught the off-by-one and the values were corrected to 8 + 9. The cap formula uses `predictionDepth` (`localFrame - 1 - highestRemoteFrameSeen`), so the 8th advance is the LAST within-cap tick (localFrame=7 → advance to 8, aheadBy = max(0,7-1-(-1)) = 7, no cap), and the 9th advance is the FIRST over-cap tick (localFrame=8, aheadBy=8, ≥ 8 ✓). Lesson: when cap math uses a derived formula, trace the threshold against the EXACT formula in the code, not against the conceptual model.
